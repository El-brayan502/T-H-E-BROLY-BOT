...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  setTimeout(() => {
    ...
    if (win) {
      users.limit += limit
      conn.reply(m.chat, `🚩 ¡Ganaste! Obtuviste ${limit} *💎BrolyCoins*. Total: ${users.limit} *💎BrolyCoins*.`, m, rcanal)
    } else {
      users.limit -= limit
      conn.reply(m.chat, `🚩 Perdiste. Se restaron ${limit} *💎BrolyCoins*. Total: ${users.limit} *💎BrolyCoins*.`, m, rcanal)
    }
